package edu.uci.ics.tingcl2.service.idm.configs;

import edu.uci.ics.tingcl2.service.idm.logger.ServiceLogger;
import edu.uci.ics.tingcl2.service.idm.models.ConfigsModel;
...
public class Configs {
    ...
    private final String DEFAULT_TIMEOUT = "600000";
    private final String DEFAULT_EXPIRATION = "1800000";
    ...
    public Configs(ConfigsModel cm) throws NullPointerException {
        if (cm == null) {
            ...
            timeout = cm.getDatabaseConfig().get("timeout");
            if (timeout == null) {
                System.err.println("No session setting timeout found in configuration file.");
                timeout = DEFAULT_TIMEOUT;
            } else {
                System.err.println("Session settings: " + timeout);
            }
            expiration = cm.getDatabaseConfig().get("expiration");
            if (expiration == null) {
                System.err.println("No session setting timeout found in configuration file.");
                expiration = DEFAULT_EXPIRATION;
            } else {
                System.err.println("Session settings: " + expiration);
            }
